...
for license in licenses:
    ...
    res = re.findall('<div property="spdx:licenseText" class="license-text">(.*?)</div>', data, re.MULTILINE | re.DOTALL)
    license = ' '.join(re.sub('<[^<]+?>', '', h.unescape(res[0])).split())


    res = re.findall('<div property="spdx:standardLicenseHeader" class="license-text">(.*?)</div>', data, re.MULTILINE | re.DOTALL)
    header = ' '.join(re.sub('<[^<]+?>', '', h.unescape(res[0])).split())
