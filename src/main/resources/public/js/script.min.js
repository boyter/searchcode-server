...
[];d.activelangfilters=[];d.activerepositoryfilters=[];d.activeownfilters=[];d.pages=[];d.currentlyloading=m.prop(!1);d.currentpage=m.prop(0);if(window.localStorage){var c=JSON.parse(localStorage.getItem("toggleinstant"));d.filterinstantly=null!==c?c:!0}else d.filterinstantly=!0;d.clearfilters=function(){d.langfilters=[];d.repositoryfilters=[];d.ownfilters=[]};d.toggleinstant=function(){window.localStorage&&localStorage.setItem("toggleinstant",JSON.stringify(!d.filterinstantly));d.filterinstantly=
...
function(a){d.coderesults.push(new testing.CodeResult(a))});_.each(a.repoFacetResults,function(a){d.repofilters.push(new testing.RepoFilter(a))});_.each(a.languageFacetResults,function(a){d.languagefilters.push(new testing.LanguageFilter(a))});_.each(a.repoOwnerResults,function(a){d.ownerfilters.push(new testing.OwnerFilter(a))});d.currentlyloading(!1);m.redraw()};null!==k?E(k):m.request({method:"GET",background:!0,url:u}).then(function(a){lruAppCache.setItem(u,a,{expirationAbsolute:null,expirationSliding:60,
...
ownfilters:testing.vm.ownfilters}),m.component(SearchAlternateFilterComponent,{query:testing.vm.query,altquery:testing.vm.altquery}),m.component(SearchRepositoriesFilterComponent,{repofilters:testing.vm.repofilters,search:testing.vm.search,filterinstantly:testing.vm.filterinstantly}),m.component(SearchLanguagesFilterComponent,{languagefilters:testing.vm.languagefilters,search:testing.vm.search,filterinstantly:testing.vm.filterinstantly}),m.component(SearchOwnersFilterComponent,{ownerfilters:testing.vm.ownerfilters,
search:testing.vm.search,filterinstantly:testing.vm.filterinstantly}),m.component(FilterOptionsComponent,{filterinstantly:testing.vm.filterinstantly})]),m("div.col-md-9.search-results",[m.component(SearchNoResultsComponent,{totalhits:testing.vm.totalhits,query:testing.vm.query,altquery:testing.vm.altquery,query:testing.vm.query}),m.component(SearchResultsComponent,{coderesults:testing.vm.coderesults})]),m.component(SearchPagesComponent,{currentpage:testing.vm.currentpage,pages:testing.vm.pages,search:testing.vm.search})])])}},
SearchNoResultsComponent={controller:function(){return{doaltquery:function(d){testing.vm.searchvalue(d);testing.vm.search()}}},view:function(d,c){if(0!==c.totalhits)return m("div");var a=m("h5","Try searching with fewer and more general keywords or if you have filters remove them.");0!==c.altquery.length&&(a="Try one of the following searches instead",1===c.altquery.length&&(a="Try the following search instead"),a=m("div",[m("h5",a),m("ul",{style:{"list-style-type":"none"}},_.map(c.altquery,function(a){return m("li",
m("a",{href:"",onclick:function(){d.doaltquery(a)}},a))}))]));return m("div",[m("h4","No results found for ",m("i.grey",c.query)),a])}},SearchNextPreviousComponent={controller:function(){},view:function(d,c){if(void 0===c.pages||void 0===c.totalhits||0===c.totalhits)return m("div");var a="",e="";0==c.currentpage()&&(a="disabled");c.currentpage()+1>=c.pages.length&&(e="disabled");return m("div",[m("h5","Page "+(c.currentpage()+1)+" of "+(0==c.pages.length?1:c.pages.length)),m("div.center",m("input.btn.btn-xs.btn-success.filter-button",
{type:"submit",disabled:a,onclick:function(){c.search(c.currentpage()-1)},value:"\u25c0 Previous"}),m("span",m.trust("&nbsp;")),m("input.btn.btn-xs.btn-success.filter-button",{type:"submit",disabled:e,onclick:function(){c.search(c.currentpage()+1)},value:"Next \u25b6"}))])}},SearchLoadingComponent={controller:function(){},view:function(d,c){var a={};!1===c.currentlyloading()&&(a={style:{display:"none"}});return m("div.search-loading",a,[m("img",{src:"/img/loading.gif"}),m("h5","Loading...")])}},SearchPagesComponent=
{controller:function(){},view:function(d,c){return m("div.search-pagination",m("ul.pagination",[_.map(c.pages,function(a){return m("li",{"class":a==c.currentpage()?"active":""},m("a",{onclick:function(){c.search(a);window.scrollTo(0,0)}},a+1))})]))}},SearchButtonFilterComponent={controller:function(){},view:function(d,c){return void 0===c.totalhits||0===c.totalhits&&0===c.languagefilters.length+c.repofilters.length+c.ownfilters.length?m("div"):0===c.totalhits&&0!==c.languagefilters.length+c.repofilters.length+
c.ownfilters.length?m("div",[m("h5","Filter Results"),m("input.btn.btn-xs.btn-success.filter-button",{type:"submit",onclick:function(){c.clearfilters();c.search()},value:"Remove"})]):m("div",[m("h5","Filter Results"),m("div.center",m("input.btn.btn-xs.btn-success.filter-button",{type:"submit",onclick:function(){c.clearfilters();c.search()},value:"Remove"}),m("span",m.trust("&nbsp;")),m("input.btn.btn-xs.btn-success.filter-button",{type:"submit",onclick:function(){c.search()},value:"Apply"}))])}},
FilterOptionsComponent={view:function(d,c){var a={type:"checkbox",onclick:function(){testing.vm.toggleinstant()}};c.filterinstantly&&(a.checked="checked");return m("div",m("h5","Filter Options"),m("div.checkbox",m("label",[m("input",a),m("span","Apply Filters Instantly")])))}},SearchRepositoriesFilterComponent={controller:function(){var d=!1,c="";return{trimrepo:function(a){0!==c.length||d||(a=_.first(a,5));0!==c.length&&(a=_.filter(a,function(a){return-1!==a.source().toLowerCase().indexOf(c)}));
return a},toggleshowall:function(){d=!d},showall:function(){return d},trimlength:function(){return 5},clickenvent:function(a){testing.vm.togglefilter("repo",a)},filtervalue:function(a){c=a},hasfilter:function(){return 0!==c.length},getfiltervalue:function(){return c}}},view:function(d,c){var a=m("div");if(void 0===c.repofilters||0==c.repofilters.length)return a;!d.hasfilter()&&d.trimlength()<c.repofilters.length&&(a=c.repofilters.length-d.trimlength(),a=m("a.green",{onclick:d.toggleshowall},a+" more repositories ",
m("span.glyphicon.glyphicon-chevron-down")),d.showall()&&(a=m("a.green",{onclick:d.toggleshowall},"less repositories ",m("span.glyphicon.glyphicon-chevron-up"))));return m("div",[m("h5","Repositories"),m("input.repo-filter",{onkeyup:m.withAttr("value",d.filtervalue),placeholder:"Filter Repositories",value:d.getfiltervalue()}),_.map(d.trimrepo(c.repofilters),function(a,k){return m.component(FilterCheckboxComponent,{onclick:function(){d.clickenvent(a.source());c.filterinstantly&&c.search()},value:a.source(),
count:a.count(),checked:testing.vm.filterexists("repo",a.source())})}),a])}},SearchLanguagesFilterComponent={controller:function(){var d=!1,c="";return{trimlanguage:function(a){0!==c.length||d||(a=_.first(a,5));0!==c.length&&(a=_.filter(a,function(a){return-1!==a.language().toLowerCase().indexOf(c)}));return a},toggleshowall:function(){d=!d},showall:function(){return d},trimlength:function(){return 5},clickenvent:function(a){testing.vm.togglefilter("language",a)},filtervalue:function(a){c=a},hasfilter:function(){return 0!==
...
m("input.repo-filter",{onkeyup:m.withAttr("value",d.filtervalue),placeholder:"Filter Languages",value:d.getfiltervalue()}),_.map(d.trimlanguage(c.languagefilters),function(a,k){return m.component(FilterCheckboxComponent,{onclick:function(){d.clickenvent(a.language());c.filterinstantly&&c.search()},value:a.language(),count:a.count(),checked:testing.vm.filterexists("language",a.language())})}),a])}},SearchOwnersFilterComponent={controller:function(){var d=!1,c="";return{trimlanguage:function(a){0!==
